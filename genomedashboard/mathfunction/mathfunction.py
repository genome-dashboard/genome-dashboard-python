...
def helix_shear_RD(Rise,Twist,V1,V2,step_number,step_size):
    ...
    for s in range(step_number):
        ...
        d = np.dot(np.array([[1,0,0],[0,np.cos(phi),-np.sin(phi)],[0,np.sin(phi),np.cos(phi)]]).T,np.dot(np.array([[np.cos(Twist*s),-np.sin(Twist*s),0],[np.sin(Twist*s),np.cos(Twist*s),0],[0,0,1]]).T,np.array([[np.cos(s*2*np.pi/V1),0,-np.sin(s*2*np.pi/V1)],[0,1,0],[np.sin(s*2*np.pi/V1),0,np.cos(s*2*np.pi/V1)]])))
        rd.append(ds.RD(r,d))
    return rd
